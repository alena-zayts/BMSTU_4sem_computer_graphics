...
class MyWindow(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def add_segment_point(self, point):
        # получаем цвет отрезков только 1 раз!
        if not self.color_segment:
            self.get_color_segment()

        if not self.segment_prev:
            self.draw_segments([[point[0], point[1], point[0], point[1]]])
            #self.segments.append([point[0], point[1]])
            ...
            self.draw_segments([[self.segment_prev[0], self.segment_prev[1], point[0], point[1]]])
            self.segments.append([self.segment_prev[0], self.segment_prev[1], point[0], point[1]])
            # добавление в таблицу
            i = self.table.rowCount()
    ...
    def add_cutter_point(self, point):
        ...
        if distance_x < distance_y:
            ...
            if len(self.cutter_tmp) == 4:
                self.handle_error('Ошибка.', 'Допустим только регулярный отсекатель. '
                                             'Нажмите кнопку "Замкнуть контур."')
                return
