...
def midpoint_cut(cutter, p1, p2, eps):
    ...
    # если отрезок не является полностью видимым
    if t1 or t2:
        # если отрезок тривиально (полностью) невидим, конец
        if t1 & t2:
            return
        # Запоминание вершины p1
        p_tmp, t_tmp = p1, t1
        for _ in range(2):
            # поиск первой вершины видимой части
            # если p2 уже видима,
            # то занесение ее в результат и переход к поиску второй вершины видимой части
            if not t2:
                answer.extend(p2)
            # иначе - поиск первой точки пересечения
            else:
                while distance(p1, p2) > eps:
                    ...
                    # иначе приближаемся к точке пересечения c другой стороны
                        ...
                        p1, t1 = pm, tm
                # занесение первой точки пересечения в результат
                answer.extend(p1)

            # обмен вершин местами
            p1, t1 = p2, t2
            p2, t2 = p_tmp, t_tmp
    # если отрезок полностью видим, то заносим в результат p1, p2
    else:
...
if __name__ == '__main__':
    print(midpoint_cut([2, 5, 4, 1], [1, 2], [6, 2], 2 ** 0.5))